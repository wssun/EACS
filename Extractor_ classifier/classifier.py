...
def main(language):
    ...
    parser.add_argument("--load_model_path", default=f'model/{language}/checkpoint-best-acc/pytorch_model.bin', type=str,
                        help="Path to trained model: Should contain the .bin files")

    parser.add_argument("--train_filename", default=f'../dataset/codesearchnet/{language}-cls/train.jsonl', type=str,
                        help="The train filename. Should contain the .jsonl files for this task.")
    parser.add_argument("--dev_filename", default=f'../dataset/codesearchnet/{language}-cls/valid.jsonl', type=str,
                        help="The dev filename. Should contain the .jsonl files for this task.")
    parser.add_argument("--test_filename", default=f'../dataset/codesearchnet/{language}-cls/test.jsonl', type=str,
                        help="The test filename. Should contain the .jsonl files for this task.")

    parser.add_argument("--max_word_length", default=32, type=int, help="")
    ...
    parser.add_argument("--do_save_ex", default=True,
                        help="")
    ...
    if args.do_test:
        ...
        if args.train_filename is not None:
            files.append(args.train_filename)
...
def save_ex_stat(filename, preds, max_stat_len):
    ...
    output_filename = f'../dataset/codesearchnet_preds/{language}/{paths[-1]}'
    with open(filename, encoding="utf-8") as f, jsonlines.open(output_filename, 'w') as writer:
        pred_idx = 0
        for idx, line in tqdm(enumerate(f)):
            line = line.strip()
            js = json.loads(line)
